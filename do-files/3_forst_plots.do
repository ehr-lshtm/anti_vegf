...
replace order = 5 if outcome =="Atrial fibrillation"
replace order = 6 if outcome =="Kidney failure" 
replace order = 7 if outcome =="Myocardial infarction" 
replace order = 4 if outcome =="Nephrotic syndrome" 
replace order = 8 if outcome =="Stroke" 
replace order = 9 if outcome =="Heart failure" 
replace order = 10 if outcome =="Peripheral arterial disease" 
replace order = 11 if outcome =="Cardiovascular death" 
replace order = 13 if outcome =="Zoster infection (negative control)" 
replace order = 12 if outcome =="Incident hypertension" 
sort group order
...
forvalues i=1/5 {
	metan hr_log lci_log uci_log if strata_n==`i' & group=="No diabetes", eform  ///
	effect(Hazard Ratio) notable forestplot(null(1) dp(2) xlab(.25 .5 1 2 3, force) favours("Favours Anti-VEGF             "   #   "             Favours control", nosymmetric) xtitle(, size(tiny)) graphregion(margin(zero) color(white)) texts(100) astext(65)) by(outcome) nowt nosubgroup nooverall nobox scheme(sj) label(namevar=strata)  
